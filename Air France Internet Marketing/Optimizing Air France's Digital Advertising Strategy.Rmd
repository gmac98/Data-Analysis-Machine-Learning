...
Using advanced visualization tools, I will create a story based on the data to uncover insights on
...
# Set colors (optional, customize as desired)
set.seed(123)  # For reproducibility
colors <- rainbow(nrow(category_freq))

# Create the wordcloud
wordcloud(names(category_freq), sort(category_freq, decreasing = TRUE), 
         min.freq = 1, colors = colors, max.words=100, random.order=FALSE)
```

ANALYSIS:
The term 'airfare' appears as the most prominent word in the cloud, suggesting that it is one of the most frequent categories.
...
# This selects rows where all columns have values
all_complete <- complete.cases(numeric_data)

# Select all columns from those rows
numeric_data_complete <- numeric_data[all_complete,]
